...
Branch: `agent/web-project-tracker`
...
sessions by provider project at `/me`.
...
1. `/me` has no application authentication and currently displays all private web sessions. Keep the
   app local/externally protected until user scoping is implemented.
2. The ingest route uses wildcard CORS for unpacked-extension development.
...
7. Database evolution relies on `drizzle-kit push`; there is no checked-in migration for the new fields.
...
2. Push the schema to a disposable database and verify create-key → POST → `/me` end to end.
3. Capture sanitized DOM fixtures and live-smoke both provider adapters.
4. Add application authentication and filter `/me` by the authenticated user/machine ownership.
5. Replace wildcard CORS with a configurable extension-origin allowlist and add ingest rate limits.
6. Add formal migrations before sharing a database.
